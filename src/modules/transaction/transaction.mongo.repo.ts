...
export class TransactionMongoRepo implements TransactionRepo {
    ...
    public findById = async (id: string): Promise<Transaction<string, string, string> | null> => {
        const DOCUMENT = await this.model.findById(id);
        return DOCUMENT?.toJSON() || null;
    };

    public update = async (transaction: Transaction<string, string, string>): Promise<boolean> => {
